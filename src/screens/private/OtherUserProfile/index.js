...
export default ({ navigation, ...rest }) => {
  ...
  const getCommonFriendsData = () => {
    ...
    while (randomIndices.length < 3) {
      const randomIndex = Math.floor(Math.random() * commonFriendIds.length);
    ...
    <SafeAreaView>
      <ScrollView>
        <VStack flex={1} px="4" pt="4" space={2}>
          <HStack pr="4" justifyContent="space-evenly" alignItems="center">
            <Avatar
              bg="purple.600"
              alignSelf="center"
              size="xl"
              source={getProfile().avatarUrl}
            />
          ...
          <HStack justifyContent="space-evenly">
            {gallery.map(({ uri }) => (
              <Image source={{ uri: uri }} alt="Alternate Text" size="xl" />
            ))}
          </HStack>
          <Text fontSize="3xl" color="muted.700">
            Amigos em comum
          ...
          {getComments().map((user) => (
            <HStack alignItems="center" key={user.id}>
              <Avatar bg="purple.600" size="64px" source={user.avatarUrl} />
              <VStack pl="6">
                <Text fontSize="sm" fontWeight="bold" color="muted.600">
                  {user.fullName}
                </Text>
